...
class MainController:
    ...
    def start(self, mode: str = None, auto_trade: bool = True):
        ...
        """
        # Wenn der Bot bereits läuft, wird dies jetzt nur bestätigt.
        if self.state == BotState.RUNNING:
    ...
    def stop(self):
        ...
        try:
            ...
            # Telegram-Interface läuft weiter für Remote-Steuerung
            self.state = BotState.READY
    ...
    def _handle_error_event(self, error_data: Dict[str, Any]):
        ...
        if level == 'critical':
            self._send_notification(f"Kritischer Fehler in {module}", message, priority="high")
    ...
    def _process_transcript(self, transcript_path: str):
        try:
            ...
            if not os.path.exists(transcript_path):
                return {'status': 'error', 'message': f"Transkript-Datei nicht gefunden: {transcript_path}"}
            if hasattr(self.transcript_processor, 'process_transcript'):
                ...
                self._send_notification("Transkript verarbeitet",
    ...
    def _get_uptime(self) -> str:
        return "00:00:00"  # Hier kann später die echte Laufzeit berechnet werden
    ...
    def generate_report(self) -> Dict[str, Any]:
        try:
            ...
            return {'status': 'error', 'message': f"Fehler: {str(e)}", 'timestamp': datetime.datetime.now().isoformat()}
